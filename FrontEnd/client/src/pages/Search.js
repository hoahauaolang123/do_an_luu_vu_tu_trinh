...
class Search extends Component {
  ...
  handleClickRate(value) {
    const { searchKey, fromPrice, toPrice, categoryId, currentPage } =
      this.props;
  ...
  render() {
    const { products, categories, totalColumns, currentPage, pageSize } =
      this.props;
      ...
      <div style={{ background: "#f7f7f7", minHeight: 480 }}>
        <div style={{ width: "75%", margin: "0 auto" }}>
          ...
          <Row gutter={{ xs: 4, sm: 8, md: 12, lg: 16 }}>
            <Col lg={{ span: 6 }}>
              ...
              >
                ...
                <span onClick={(value) => this.handleClickRate(5)}>
                  <Rate value={5} disabled></Rate> (từ 5 sao)
                </span>
                <span onClick={(value) => this.handleClickRate(4)}>
                  <Rate value={4} disabled></Rate> (từ 4 sao)
                </span>
                <span onClick={(value) => this.handleClickRate(3)}>
                  <Rate value={3} disabled></Rate> (từ 3 sao)
                </span>
                <form onSubmit={this.handleSearchByPrice.bind(this)}>
                  <h5>GIÁ</h5>
                  <InputNumber name="fromPrice"></InputNumber>{" "}
                  <ArrowRightOutlined />{" "}
                  <InputNumber name="toPrice"></InputNumber>
                  <div style={{ width: 100, margin: "10px 0" }}>
                    <Button htmlType="submit" type="primary" ghost block>
                      OK
                    </Button>
                  </div>
                </form>
            ...
            <Col lg={{ span: 18 }}>
              <div
                style={{ background: "#ffffff", padding: 20, minHeight: 400 }}
              >
                {products.length ? (
                  <>
                    <div
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    searchProduct: (query) => {
      dispatch(fetch_search_product(query));
    },
  };
};
