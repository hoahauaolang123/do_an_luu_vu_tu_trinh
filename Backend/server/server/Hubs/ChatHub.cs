...
namespace server.Hubs
{
    ...
    public class ChatHub: Hub
    {
        ...
        {
            var response = await _chatService.CreateMessageV2(_message);
           
            var messageId = response.Item1;
            var sender = response.Item2;
            ...
            if (ReceiverConnectionids.Count() > 0)
            {
                ...
                try
                {
                    var notify = new NotifyViewModel() {
                        notify = $"Bạn đã nhận tin nhắn từ {sender.displayname}",
                        link = null,
