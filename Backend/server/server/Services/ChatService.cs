...
namespace server.Services
{
    public class ChatService : IChatService
    {
        ...
        public async Task<int> CreateMessage(ChatCreateRequest request)
        {
            try
            {
                var checkSender = await _context.Users.SingleOrDefaultAsync(x => x.Id == request.senderId);
        ...
        public async Task<(int,AppUser)> CreateMessageV2(ChatCreateRequest request)
        {
            try
            {
                var checkSender = await _context.Users.SingleOrDefaultAsync(x => x.Id == request.senderId);
